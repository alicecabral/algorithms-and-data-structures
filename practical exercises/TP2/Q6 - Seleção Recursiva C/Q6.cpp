...
void selecaoRecursiva(Jogador *array, int i, int n){
    if(i < n){
        int menor = i;
        for (int j = (i + 1); j < n; j++){
            comp++;
...
int main(){
    ...
    selecaoRecursiva(jogadores2, 0, numEntrada);
    ...
    FILE *file;
    file = fopen("589690_selecaoRecursiva.txt", "w");
